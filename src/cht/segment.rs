...
impl<K, V, S> HashMap<K, V, S> {
  ...
  pub(crate) fn with_num_segments_capacity_and_hasher(
    num_segments: usize, capacity: usize, build_hasher: S,
  ) -> Self {
    ...
    Self { segments, build_hasher, len: AtomicUsize::new(0), segment_shift }
  }
...
impl<K: Hash + Eq, V, S: BuildHasher> HashMap<K, V, S> {
  ...
  pub(crate) fn get_key_value_and<T>(
    &self, hash: u64, eq: impl FnMut(&K) -> bool, with_entry: impl FnOnce(&K, &V) -> T,
  ) -> Option<T> {
  ...
  pub(crate) fn get_key_value_and_then<T>(
    &self, hash: u64, eq: impl FnMut(&K) -> bool, with_entry: impl FnOnce(&K, &V) -> Option<T>,
  ) -> Option<T> {
    self.bucket_array_ref(hash).get_key_value_and_then(hash, eq, with_entry)
  }
  ...
  pub fn insert_entry_and<T>(
    &self, key: K, hash: u64, value: V, with_previous_entry: impl FnOnce(&K, &V) -> T,
  ) -> Option<T>
  where V: Clone {
    let result = self
      .bucket_array_ref(hash)
      // .insert_entry_and(key, hash, value, with_previous_entry);
      .insert_with_or_modify_entry_and(key, hash, || value, |_k, v| v.clone(), with_previous_entry);
  ...
  pub(crate) fn remove_if(
    &self, hash: u64, eq: impl FnMut(&K) -> bool, condition: impl FnMut(&K, &V) -> bool,
  ) -> Option<V>
  where V: Clone {
  ...
  pub(crate) fn remove_entry_if_and<T>(
    ...
    self.bucket_array_ref(hash).remove_entry_if_and(hash, eq, condition, move |k, v| {
  ...
  pub(crate) fn insert_with_or_modify(
    &self, key: K, hash: u64, on_insert: impl FnOnce() -> V, on_modify: impl FnMut(&K, &V) -> V,
  ) -> Option<V>
  where V: Clone {
  ...
  where V: Clone {
    let result =
      self.bucket_array_ref(hash).insert_if_not_present_and(key, hash, || value, |_, v| v.clone());
  ...
  pub(crate) fn keys<T>(&self, segment: usize, with_key: impl FnMut(&K) -> T) -> Option<Vec<T>> {
    ...
    let Segment { ref bucket_array, ref len } = self.segments[segment];

    let bucket_array_ref = BucketArrayRef { bucket_array, build_hasher: &self.build_hasher, len };
...
impl<K, V, S> Drop for HashMap<K, V, S> {
  fn drop(&mut self) {
    ...
    for Segment { bucket_array: this_bucket_array, .. } in self.segments.iter() {
...
impl<K, V, S> HashMap<K, V, S> {
  ...
  fn bucket_array_ref(&'_ self, hash: u64) -> BucketArrayRef<'_, K, V, S> {
    ...
    let Segment { ref bucket_array, ref len } = self.segments[index];

    BucketArrayRef { bucket_array, build_hasher: &self.build_hasher, len }
  }
...
mod tests {
  ...
  #[test]
  fn single_segment() {
    let map = HashMap::with_num_segments_capacity_and_hasher(1, 0, DefaultHashBuilder::default());
  ...
  fn concurrent_insert_if_not_present() {
    ...
    // Initialize results2.
    let mut results2 =
      (0..NUM_THREADS).map(|thread_id| (thread_id, 0usize)).collect::<BTreeMap<_, _>>();
  ...
  fn concurrent_insertion_and_removal() {
    ...
    for result in insert_threads.into_iter().chain(remove_threads.into_iter()).map(|t| t.join()) {
      assert!(result.is_ok());
    }
  ...
  fn concurrent_growth_and_removal() {
    ...
    for result in insert_threads.into_iter().chain(remove_threads.into_iter()).map(JoinHandle::join)
    {
      assert!(result.is_ok());
    }
  ...
  fn insert_with_or_modify() {
    ...
    assert_eq!(map.insert_with_or_modify(key, hash, || 1, |_, x| x + 1), None);
    assert_eq!(map.get(hash, |&k| k == key), Some(1));

    assert_eq!(map.insert_with_or_modify(key, hash, || 1, |_, x| x + 1), Some(1));
  ...
  fn drop_many_values() {
    ...
    let key_parents: Vec<_> =
      std::iter::repeat_with(|| Arc::new(DropNotifier::new())).take(NUM_VALUES).collect();
    let value_parents: Vec<_> =
      std::iter::repeat_with(|| Arc::new(DropNotifier::new())).take(NUM_VALUES).collect();

    {
      ...
      let live_key_count = NUM_VALUES - key_parents.iter().filter(|k| k.was_dropped()).count();
      ...
      for i in 0..NUM_VALUES {
        assert_eq!(map.get_key_value_and(map.hash(&i), |k| k == &i, |_, _| ()), None);
      }
  ...
  fn drop_many_values_concurrent() {
    ...
    let key_parents: Arc<Vec<_>> =
      Arc::new(std::iter::repeat_with(|| Arc::new(DropNotifier::new())).take(NUM_VALUES).collect());
    let value_parents: Arc<Vec<_>> =
      Arc::new(std::iter::repeat_with(|| Arc::new(DropNotifier::new())).take(NUM_VALUES).collect());
      ...
      let handles: Vec<_> = (0..NUM_THREADS)
        .map(|i| {
          ...
          spawn(move || {
            ...
            for (j, (this_key_parent, this_value_parent)) in
              these_key_parents.iter().zip(these_value_parents.iter()).enumerate()
            {
